...
let seq_container: Chemistry.Geometry[] = [];
let seq: number = 0;
let valve: Chemistry.Custome_image;
var ta;

let first_motor_anim: Chemistry.anim_image;
let second_motor_anim: Chemistry.anim_image_x_dir;
let third_motor_anim: Chemistry.anim_image_y_dir_down;
let first_sim: Chemistry.Custome_image;
let second_sim: Chemistry.anim_image;
let third_sim: Chemistry.anim_image_x_dir_slow;
let fourth_sim: Chemistry.anim_image_y_dir_down;
let fifth_sim: Chemistry.anim_image_x_dir;

let pump_power: Chemistry.Circle;
let heater_power: Chemistry.Circle;
let pump_text: Chemistry.Geo_Text;
let heater_text: Chemistry.Geo_Text;


let floater: Chemistry.Custome_image;

let tin: Chemistry.Geo_Text;
let tout: Chemistry.Geo_Text;

let timer: Chemistry.Custome_image;
let timer_text: Chemistry.Geo_Text;
let heater_dsp: Chemistry.Geo_Text;

let heater_temp: number = 0.0;
let can_start_timer: boolean = false;
...
let fr_text: Chemistry.Geo_Text;
let tin_label: Chemistry.Geo_Text;
let tout_label: Chemistry.Geo_Text;
let temp_label: Chemistry.Geo_Text;
...
function motor_event(e) {
    ...
    if(x >= 810 && x <= 887) {
        if(y >= 811 && y <= 875) {
            ...
            slider = <HTMLInputElement> document.getElementById('float-slider1');
            slider.value = '1';
            motor_on_animation();
